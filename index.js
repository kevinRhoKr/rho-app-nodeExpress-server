...
app.post("/user/signup", async (req, res) => {
  ...
  try {
    ...
    const user = await pool.query("SELECT f_name FROM appUsers WHERE email = $1", [email]);
...
app.post("/user", async (req, res) => {
  try {
    ...
    const user = await pool.query("SELECT email, f_name, l_name FROM appUsers WHERE email = $1",
      [email]);
    console.log(user);
    res.json(user.rows[0]);
  } catch (err) {
